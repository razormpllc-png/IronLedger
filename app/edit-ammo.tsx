...
export default function EditAmmoScreen() {
  ...
  useEffect(() => {
    if (!id) return;
    try {
      const ammo = getAmmoById(Number(id));
      if (ammo) {
        setCaliber(ammo.caliber);
        setBrand(ammo.brand || '');
        setGrain(ammo.grain ? String(ammo.grain) : '');
        setType(ammo.type || null);
        setQuantity(String(ammo.quantity));
        setRoundsPerBox(ammo.rounds_per_box ? String(ammo.rounds_per_box) : '50');
        setCostPerBox(ammo.cost_per_box ? String(ammo.cost_per_box) : '');
  ...
  function togglePaired(fId: number) {
    setPairedIds((prev) => prev.includes(fId) ? prev.filter(x => x !== fId) : [...prev, fId]);
  }
  ...
  function handleSave() {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to update ammo');
    }
  }
